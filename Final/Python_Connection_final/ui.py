...
def send_message():
    ...
    if c == True :
        ...
        messagebox.showinfo("Thank you for contacting us\n ", order_summary)
        send_email_notify(email,"Thank You For Contacting will connect to you soon !!")
    else :
        print("Check Email or Phone Number")
        order_summary = f"Name: {name}\n Email: {email}\nPhone Number: {phone_number}\n Message : {message} \n Please ReCheck The Information\n Phone number must be 10 Digits \n Adress must only Contain Alphabets"
        messagebox.showinfo("Error In Information\n ", order_summary)
        send_email_notify(email,"Sorry we Did not get Your Message You could Conatct us on this email")
...
def place_order():
    ...
    item = item_entry.get()
    email = email_en.get()
    v = verify(phoneSender,phone_number,pickup,address,email)
    if v == True :
        ...
        if check == True:
            ...
            send_email_notify(email,"Order Successful")
            time.sleep(5)
            generate_html(name,100,pickup,address,email)
        else:
            order_summary = f"Name: {name}\nAddress: {address}\nPhone Number: {phone_number}\n Pickup Address : {pickup} \n Phone Number of Sender : {phoneSender} \n Please ReCheck Address"
            # button = tk.Button(root, text="Show Image", command=show_map)
            # # button = tk.Button(root, text="Show Image")
            # button.pack()
            send_email_notify(email,"Order failed Check Details")
...
if __name__ == "__main__" :
    ...
    email_la = ttk.Label(order_frame, text="Email ID",background='#011533',foreground='white')
    email_la.grid(row=6, column=0, padx=10, pady=10)
    email_en = ttk.Entry(order_frame)
    email_en.grid(row=6, column=1, padx=10, pady=10)

    order_button = ttk.Button(order_frame, text="Place Order", command=place_order)
    order_button.grid(row=7, columnspan=2, padx=10, pady=10)
    # Add an image
